...
/// OneDrive 发送实现（带 mock 支持，供测试用）。
///
/// `graph_api` 为 `None` 时使用线上端点；测试传入 wiremock 地址。
pub async fn send_with_base(
    token: &str,
    local_path: &str,
    remote_path: &str,
    graph_api: Option<&str>,
) -> Result<String, String> {
    let data = fs::read(local_path).map_err(|e| format!("读取文件失败: {e}"))?;
    let client = Client::new();
    let graph_api = graph_api.unwrap_or(GRAPH_API).to_string();
    let size = data.len();
...
/// OneDrive 接收实现（带 mock 支持，供测试用）。
pub async fn receive_with_base(
    token: &str,
    url: &str,
    local_path: &str,
    _graph_api: Option<&str>,
) -> Result<(), String> {
    let client = Client::new();
...
impl StorageProvider for OneDriveProvider {
    ...
    async fn send(&self, local_path: &str, remote_path: &str) -> Result<String, String> {
        ...
        send_with_base(&token, local_path, remote_path, None).await
    }
