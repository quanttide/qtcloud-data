...
/// Google Drive 发送实现（带 mock 支持，供测试用）。
///
/// `api_base` / `upload_base` 为 `None` 时使用线上端点；测试传入 wiremock 地址。
pub async fn send_with_base(
    ...
    let api_base = api_base.unwrap_or(API_BASE).to_string();
    let upload_base = upload_base.unwrap_or(UPLOAD_BASE).to_string();
    let file_name = remote_path.rsplit('/').next().unwrap_or("file");
    let mime = mime_guess::from_path(local_path)
        .first_or_octet_stream()
        .to_string();

    // 1. 先创建文件元数据（空 body），获取上传 URL
...
/// Google Drive 接收实现（带 mock 支持，供测试用）。
...
impl StorageProvider for GoogleDriveProvider {
    ...
    async fn send(&self, local_path: &str, remote_path: &str) -> Result<String, String> {
        ...
        send_with_base(&token, local_path, remote_path, None, None).await
    }

    async fn receive(&self, url: &str, local_path: &str) -> Result<(), String> {
        let token = self.token()?;
        receive_with_base(&token, url, local_path, None).await
    }
}
