...
> `main` `[Unreleased]` 已包含 CLI 规格流、交付记录和 Provider Blueprint runner 底座，尚未发布到 crates.io/GitHub Release。
...
### main `[Unreleased]`：Provider Blueprint runner 底座
...
- [ ] Studio 通过 Provider API 浏览 Blueprint 列表、详情和执行记录
- [ ] Studio 在 Blueprint 详情页接入 run 表单，调用 Provider `POST /blueprints/{name}/runs`
- [ ] Studio 执行记录支持点击进入 job 详情，查看 step 输入、输出和状态
...
- [ ] 将 `main` `[Unreleased]` 内容切分为可发布版本，并同步 crates.io/GitHub Release
